      ...
      <Column field="fullName" header="Họ tên" sortable style="min-width: 14rem">
        <template #body="{ data }">
          <div class="flex items-center">
            <img 
              :src="getAvatarUrl(data.avatar) || 'https://placehold.co/100x100/EEE/999?text=No+Avatar'" 
              :alt="data.fullName"
      ...
      <Column header="Vai trò" style="min-width: 12rem">
        <template #body="{ data }">
          <div class="flex flex-wrap gap-1">
            ...
            <span v-if="!userRoles[data.id] || userRoles[data.id].length === 0" class="text-gray-500">
              Không có vai trò
            </span>
          </div>
        </template>
