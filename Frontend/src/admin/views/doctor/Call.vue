          ...
          <div class="video-container">
            <div class="status-bar">
              ...
              <div class="call-controls">
                ...
                <button class="control-btn video" :class="{ 'disabled': !videoActive }" @click="toggleVideo">
              ...
              <div v-for="(message, index) in messages" :key="index"
                ...
                <div class="avatar-container">
                  <img :src="message.sender === 'doctor' ? doctorAvatar : userData.avatar"
                    :alt="message.sender === 'doctor' ? 'Doctor avatar' : 'Patient avatar'" class="message-avatar" />
                </div>
...
// Call states
const patientConnected = ref(false);
...
const initializeWebRTC = async () => {
  try {
    ...
    peerConnection.value.ontrack = (event) => {
      ...
      if (patientVideo.value && event.streams && event.streams[0]) {
        patientVideo.value.srcObject = event.streams[0];
        patientConnected.value = true;
        connectionStatus.value = 'Đã kết nối với bệnh nhân';
      }
    };

    console.log('WebRTC đã được khởi tạo thành công');

    // Bác sĩ luôn tạo offer trước
...
const createAndSendOffer = async () => {
  try {
    if (!peerConnection.value) {
      console.error('Không có peer connection để tạo offer');
      return;
    }
    ...
    console.log('Đã tạo và thiết lập offer');

    // Gửi offer cho bệnh nhân
    consultationService.sendWebRTCMessage({
      consultationId: consultationDetails.value.id,
      type: 'OFFER',
      sdp: peerConnection.value.localDescription
    });
    console.log('Đã gửi offer cho bệnh nhân');
  } catch (error) {
    console.error('Lỗi tạo và gửi offer:', error);
  }
};

// Xử lý answer từ bệnh nhân
const handleWebRTCAnswer = async (message) => {
  try {
    console.log('Nhận được answer từ bệnh nhân:', message);
...
// Xử lý ICE candidate từ bệnh nhân
const handleICECandidate = async (message) => {
  try {
    console.log('Nhận được ICE candidate từ bệnh nhân:', message);
...
const initializeConsultation = async () => {
  try {
    ...
    // Update patient info
    updatePatientInfo(consultationDetails.value);

    // Setup WebSocket connection
    setupWebSocketConnection();

    // Bác sĩ chỉ tham gia vào phiên đã được bệnh nhân khởi tạo
    if (consultationDetails.value.status === 'IN_PROGRESS') {
      connectionStatus.value = 'Phiên khám đã được bắt đầu, đang kết nối...';
      startTimer();
    } else {
      connectionStatus.value = 'Đang chờ bệnh nhân bắt đầu phiên khám...';
    }
  } catch (error) {
    handleConnectionError(error);
  }
...
const updatePatientInfo = (details) => {
  patientName.value = details.patientName || 'Bệnh nhân';
  patientAvatar.value = details.patientAvatar || '@/assets/images/default-patient.jpg';
...
const handleWebSocketMessage = (message) => {
  ...
  switch (message.type) {
    ...
    case 'SESSION_STARTED':
      ...
      startTimer();
      // Bác sĩ khởi tạo kết nối WebRTC và gửi offer
    ...
    case 'VIDEO_DISABLED':
      ...
      break;
    case 'AUDIO_ENABLED':
      console.log('Bệnh nhân đã bật microphone');
      break;
    case 'AUDIO_DISABLED':
      console.log('Bệnh nhân đã tắt microphone');
      break;
    case 'OFFER':
      // Bác sĩ không xử lý offer từ bệnh nhân vì bác sĩ là người gửi offer
      console.log('Nhận được offer từ bệnh nhân, bỏ qua vì bác sĩ là người gửi offer');
      break;
    case 'ANSWER':
    ...
    case 'CLIENT_CONNECTED':
      if (message.senderType === 'PATIENT') {
        connectionStatus.value = 'Bệnh nhân đã tham gia phiên khám';
      }
      break;
    default:
...
const requestMediaPermissions = async () => {
  try {
    ...
    consultationService.sendChatMessage({
      ...
      senderId: user.id,
      senderType: 'DOCTOR',
      content: 'Bác sĩ đã kết nối camera',
      messageType: 'TEXT',
      timestamp: new Date()
    });
...
const startAudioOnly = async () => {
  try {
    ...
    consultationService.sendChatMessage({
      ...
      senderId: user.id,
      senderType: 'DOCTOR',
      content: 'Bác sĩ đã chuyển sang chế độ âm thanh',
      messageType: 'TEXT',
      timestamp: new Date()
    });

    // Khởi tạo WebRTC với chỉ âm thanh
    initializeWebRTC();
  } catch (error) {
...
const handleChatMessage = (message) => {
  messages.value.push({
    sender: message.senderType === 'PATIENT' ? 'patient' : 'doctor',
...
const toggleScreenShare = async () => {
  try {
    ...
    if (localStream.value && localStream.value._isScreenShare) {
      ...
      // Cần khởi tạo lại WebRTC khi quay lại camera
    ...
    if (localStream.value) {
      ...
      consultationService.sendChatMessage({
        ...
        senderType: 'DOCTOR',
        content: 'Bác sĩ đang chia sẻ màn hình',
        messageType: 'TEXT',
        timestamp: new Date()
      });
...
const endCall = async (showConfirm = true) => {
  try {
    ...
    await consultationService.endConsultationSession({
      ...
      terminatorType: 'DOCTOR',
      terminationReason: 'Bác sĩ kết thúc cuộc gọi'
    });
...
const cleanupResources = () => {
  ...
  if (patientVideo.value) {
    patientVideo.value.srcObject = null;
  }
